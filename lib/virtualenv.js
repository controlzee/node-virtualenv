...
function VirtualEnv(packagePath) {
  ...
  packageObj.virtualenv = packageObj.virtualenv || {};
  this._version = packageObj.virtualenv.version || "*";
  this._createFlags = packageObj.virtualenv.flags || [];
  this._python = packageObj.virtualenv.python || "python";
  this._virtualEnvName = packageObj.virtualenv.envName || ".node-virtualenv";
  ...
  this._virtualenvHome = path.join(path.dirname(packagePath), this._virtualEnvName);
  this._virtualenvMetaHome = path.join(this._virtualenvHome, this._virtualEnvName + ".json");
  this._virtualenvSourcesHome = path.join(this._virtualenvHome, this._virtualEnvName + "-sources");
  ...
  // 2. Virtualenv config changes (packageObj.virtualenv)
  // 3. Postinstall script changes (packageObj.scripts.postinstall)
  ...
  md5.update([
    VERSION,
    JSON.stringify(packageObj.virtualenv),
    JSON.stringify(packageObj.scripts ? packageObj.scripts.postinstall : ""),
...
VirtualEnv.prototype.spawnSync = function spawnSync(command, args, options) {
  var pathToVirtualenvCommand = path.join(this._virtualenvHome, "bin", command);
  return childProcess.spawnSync(pathToVirtualenvCommand, args, options);
}
...
VirtualEnv.prototype.spawnPythonSync = function spawnPythonSync(args, options) {
  var pathToPython = path.join(this._virtualenvHome, "bin", "python");
  return childProcess.spawnSync(pathToPython, args, options);
}
